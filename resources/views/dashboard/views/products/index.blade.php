                ...
                {!! Form::open(['method'=>'DELETE', 'route'=> [$resource['route'].'.multiDelete', App::getLocale()], 'class'=>'delete-form'])!!}
                    ...
                    <table class="table table-hover">
                        ...
                        @foreach($data as $item)
                            @php
                                $count = \App\Models\ProductImage::where('product_id', $item->id)->count();
                            @endphp
                            <tr class="tr-{{ $item->id }}">
                              <td>{{ $item['name_'.App::getLocale()] }}</td>
                               <td>{{ $item->Department['name_'.App::getLocale()] }}</td>
                                <td>{{ $item->price }}</td>
                                <td>{!! $item['desc_'.App::getLocale()] !!}</td>
                                <td>
                                    @if($count > 0)
                                            <a href="{{ route('admin.productImages.show', [App::getLocale(), $item->id]) }}">{{ $count }}</a>
                                    @else
                                            <a href="#">{{ $count }}</a>
                                    @endif
                                </td>
